          ...
          <div class="relative flex flex-col h-full">
            {visitorData.map((val, index) => {
              ...
              return (
                <div className='flex gap-1.5'>
                  <Link to={`${path}`} onClick={() => setActive(index)} key={index} className={`hover:bg-blue-200 capitalize mt-2 w-full px-3 py-4 rounded-2xl flex gap-1.5 ${active === index ? "bg-blue-500  text-white" : ""}`}>
                    <Icon />
                    {label}
                  </Link>
                </div>
            ...
            <div class="mt-4">
              <ul class="space-y-4 px-2">
                <li>
                  <a href="javascript:void(0)" class="text-slate-800 text-sm flex items-center font-medium hover:text-blue-600 transition-all">
              ...
              <div class="flex flex-wrap items-center cursor-pointer border-t border-gray-200 py-2 mt-6">
                <img src='https://readymadeui.com/team-2.webp' class="w-10 h-10 rounded-md border-2 border-white" />
                <div class="ml-4">
                  <p class="text-sm text-slate-900 font-medium">John Doe</p>
