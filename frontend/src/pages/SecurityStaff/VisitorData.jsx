...
const VisitorData = () => {
    ...
    const { verifyPassVisitor, loading, verifyPassUser, checkInVisitor } = useContext(VisitorContext);
    ...
    const handelCheckInVisitor = async () => {
        await checkInVisitor(data?.visitorId
        );
        toast.success("done this visior check-in")
    }
        ...
        <>
            {
                data.status === "check-in" ? <div className='flex items-center justify-center flex-col gap-2.5'>
                    ...
                    <button className='px-5 py-2 rounded-xl bg-green-500 text-white cursor-pointer hover:bg-green-700 duration-200'>Allow to Check-out</button>
                </div> :
                    verifyPassUser && <div className='p-5 '>
                        ...
                        <div className='mt-5 flex items-center justify-center gap-3'>
                            <button onClick={handelCheckInVisitor} className='px-5 py-2 rounded-xl bg-green-500 text-white cursor-pointer hover:bg-green-700 duration-200'>Allow to Check-in</button>
